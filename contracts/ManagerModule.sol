...
contract ManagerModule is ModuleBase, ReentrancyGuard {
    ...
    {   
        ...
        address[] memory allowedTokens = IPortfolioFactory(_portfolioFactory).getAllowedTokens();
        uint256 allowed = 0;
        for (uint256 i = 0; i < allowedTokens.length; i++) {
            if (_receiveToken == allowedTokens[i]) {
                allowed = 1;
            }
        }
        require(allowed == 1, "Token does not exist");
